...
int main() 
{
    ...
    {
        cout << "The key " << key << " is not in the dictionary." << "\n";
        cout << "Inserting a key-value pair into the dictionary." << "\n\n";
        // Set the value for the key.
        my_map.insert({key, "Maldives"});
    }

    // Test map
    vector<int> codes{91, 1, 960};
    for (int code: codes)
    {
        cout << code <<": ";

        // It returns a pair representing the range of elements with a given key
        result = my_map.equal_range(code);
	    ...
	    cout << "Total values for key " << code << " are: " << count << std::endl;
        cout << "\n";
    }
}